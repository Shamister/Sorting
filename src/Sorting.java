...
public class Sorting {
	...
	/**
	 * public void mergeSort(String[] data) { String[] other = new
	 * String[data.length]; for(int i=0; i<data.length; i++) other[i]=data[i];
	...
	/**
	 * Iterative Merge Sort Modified Merge Sort using for loop Splitting
	 * elements into 2^n subrange and merge them The remainder elements will be
	 * merged with previous merged elements
	 */
	...
	// Challenge
	// Wrapper method, non-recursive
	public void timSort(String[] data) {
		String[] other = new String[data.length];
		for (int i = 0; i < data.length; i++)
			other[i] = data[i];
		timSort(data, other, 0, data.length); // call to recursive merge sort
												// method
	}

	public void timSort(String[] data, String[] temp, int low, int high) {
		if (low < high - 1) {
			int mid = (low + high) / 2;
			if (high - low < 3)
				if (data[low].compareTo(data[low + 1]) > 0)
					swap(data, low, low + 1);
				else if (high - low < 6) {
					insertionSort(data, low, high);
					return;
	...
	/**
	 * Sorts the elements of an array of String from 'low' to 'high' using
	 * insertion sort
	 */
	// MODIFIED
	...
	/**
	 * public String[] createArray(int size) { Random randGenerator = new
	 * Random(); String[] data = new String[size]; for (int i=0; i<size; i++){
	 * char[] chars = new char[5]; for (int c=0; c<chars.length; c++){ chars[c]
	 * = (char) ('a' + randGenerator.nextInt(26)); } String str = new
	 * String(chars); data[i] = str; } return data; }
	 */
